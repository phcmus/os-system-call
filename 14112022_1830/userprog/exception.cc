...
// Quy uoc do dai ten file toi da
...
void ExceptionHandler(ExceptionType which)
{
  ...
  case SyscallException:
    ...
    case SC_Read:
    {
      ...
      if (kernel->fileSystem->opened[id] == NULL)
      {
        printf("\nKhong the read vi file nay khong ton tai.");
        kernel->machine->WriteRegister(2, -1);
        return;
      }
      if (kernel->fileSystem->opened[id]->type ==
      ...
      OldPos = kernel->fileSystem->opened[id]
      ...
      if (kernel->fileSystem->opened[id]->type == 2)
      {
      ...
      if ((kernel->fileSystem->opened[id]->Read(buf, charcount)) > 0)
      {
        // So byte thuc su = NewPos - OldPos
        NewPos = kernel->fileSystem->opened[id]->GetCurrentPos();
    ...
    case SC_Write:
    {
      ...
      if (kernel->fileSystem->opened[id]->type == 1 ||
          kernel->fileSystem->opened[id]->type == 2)
      {
      ...
      // ve so byte thuc su
      if (kernel->fileSystem->opened[id]->type == 0)
      {
        if ((kernel->fileSystem->opened[id]->Write(buf, charcount)) >
            0)
        {
          // So byte thuc su = NewPos - OldPos
          NewPos = kernel->fileSystem->opened[id]->GetCurrentPos();
    ...
    case SC_Open:
    {
      ...
      char *filename;
      filename = User2System(vAddr, MAX_FILE_LENGTH);

      DEBUG(dbgSys, "Filename returning with :" << filename << ":\n");

      int slot = kernel->fileSystem->FindSlot();
      if (slot != -1)
      {
        if (type == 0 || type == 1)
        {
          if ((kernel->fileSystem->opened[slot] = kernel->fileSystem->Open(filename, type)) != NULL)
          {
    ...
    case SC_Close:
    {
      ...
      // Check if file_id is invalid/not available in opened files
      if (id >= 0 && id <= 9)
      {
        // Begin termination
        if (kernel->fileSystem->opened[id])
        {
          delete kernel->fileSystem->opened[id];
          kernel->fileSystem->opened[id] = NULL;
    ...
    case SC_Seek:
    {
      ...
      // Check for invalid file_id:
      // - Accessing I/O file
      // - file_id is out of opened files' list's bounds
      // - file_id points to unopened file
      if (file_id < 2 || file_id > kernel->fileSystem->index || kernel->fileSystem->opened[file_id] == NULL)
      {
        kernel->machine->WriteRegister(2, -1);
        break;
      }
      ...
      // Get filesize
      int len = kernel->fileSystem->opened[file_id]->Length();
      ...
      // Execute seek
      kernel->fileSystem->opened[file_id]->Seek(pos);
